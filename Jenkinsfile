        ...
        stage('Building image and run script') {
            steps{
                script {
                    ...
                    // bat "docker run --name evilfile ${IMAGE_REPO_NAME}:${IMAGE_TAG} /bin/bash -c 'ls ; pwd ' " //working
                    bat " docker run --name evilfile ${IMAGE_REPO_NAME}:${IMAGE_TAG} /bin/bash -c bash fix.sh "
